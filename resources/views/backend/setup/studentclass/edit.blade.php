...
@section('title', 'Edite-student-class - dashboard')
@section('content')
        ...
        <div class="col-md-12">
            <div class="card mt-3">
                <div class="card-header">
                  <h3>Update student class 
                    <a class="btn btn-primary float-right btn-sm" href="{{ route('backend.class.index') }}"><i class="fa fa-list"></i> All class</a>
                  </h3>
                </div>
                <!-- /.card-header -->
                <!-- form start -->
                <form role="form" action="{{ route('backend.class.update', $data->id) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                  <div class="card-body">
